...
for state, styleData in pairs(config.AvailabilityStyle) do
	if styleData.Enable then
		...
		if bulletList.Alpha.Override and bulletList.Alpha.Value then
			BoonColors.Requirement.BulletList[state] =
				ChangeAlpha(BoonColors.Requirement.BulletList[state], ClampAlpha(bulletList.Alpha.Value))
		end
		...
		if header and header.Alpha.Override and header.Alpha.Value then
			BoonColors.Requirement.Header[state] =
				ChangeAlpha(BoonColors.Requirement.Header[state], ClampAlpha(header.Alpha.Value))
		end
	end
end
...
modutil.mod.Path.Override("CreateBoonInfoButton", function(screen, traitName, index)
	...
	if not newTraitData.HideStatLinesInCodex then
		...
		if
			newTraitData.CustomStatLinesWithShrineUpgrade ~= nil
			and GetNumShrineUpgrades(newTraitData.CustomStatLinesWithShrineUpgrade.ShrineUpgradeName) > 0
		then
	...
	if HasStoreItemPin(traitName) then
		...
		CreateTextBox({
			Id = button.Id,
			TextSymbolScale = 0,
			Text = "NeededPinBoonTooltip_Codex",
			Color = Color.Transparent,
		})
	end

	if
		IsGameStateEligible(screen, TraitRarityData.ElementalGameStateRequirements)
		and not IsEmpty(newTraitData.Elements)
	then
	...
	if
		not GameState.TraitsTaken[traitName]
		and not GameState.ItemInteractions[traitName]
		and HasActiveQuestForName(traitName)
	then
	...
	if boonState == BoonState.Denied and CurrentHubRoom == nil then
		local bannedOverlay = CreateScreenComponent({
			Name = "BlankObstacle",
			Group = "Combat_Menu_TraitTray_Overlay",
			Animation = "BoonInfoSlotLocked",
			X = purchaseButton.X,
			Y = purchaseButton.Y,
		})
	...
	if traitToReplace ~= nil then
		...
		local exchangeSymbol = CreateScreenComponent({
			Name = "BlankObstacle",
			Group = "Combat_Menu_TraitTray_Overlay",
			Scale = screenData.ExchangeSymbol.Scale,
		})
		table.insert(traitInfo.Components, exchangeSymbol)
		Attach({
			Id = exchangeSymbol.Id,
			DestinationId = traitInfo.PurchaseButton.Id,
			OffsetX = screenData.ExchangeSymbol.OffsetX,
			OffsetY = screenData.ExchangeSymbol.OffsetY,
		})
		SetAnimation({ DestinationId = exchangeSymbol.Id, Name = "TraitExchangeSymbol" })

		local exchangeIcon = CreateScreenComponent({
			Name = "BlankObstacle",
			Group = "Combat_Menu_TraitTray_Overlay",
			Scale = screenData.Icon.Scale * screenData.ExchangeIconScale,
		})
		table.insert(traitInfo.Components, exchangeIcon)
		Attach({
			Id = exchangeIcon.Id,
			DestinationId = traitInfo.PurchaseButton.Id,
			OffsetX = screenData.ExchangeIconOffsetX,
			OffsetY = screenData.ExchangeIconOffsetY,
		})
		SetAnimation({ DestinationId = exchangeIcon.Id, Name = TraitData[traitToReplace].Icon })

		local exchangeIconFrame = CreateScreenComponent({
			Name = "BlankObstacle",
			Group = "Combat_Menu_TraitTray_Overlay",
			Scale = screenData.Icon.Scale * screenData.ExchangeIconScale,
		})
...
modutil.mod.Path.Override(
	"CreateTraitRequirementList",
	function(screen, headerTextArgs, traitList, startY, metRequirement)
		...
		for i, traitName in ipairs(traitList) do
			local traitData = TraitData[traitName]
			if
				traitData.CodexGameStateRequirements == nil
				or IsGameStateEligible(traitData, traitData.CodexGameStateRequirements)
			then
				...
				if config.IconInRequirements.Pinned.Enable and game.HasStoreItemPin(traitName) then
					local pinned = CreateScreenComponent({
						Name = "BlankObstacle",
						Group = "Combat_Menu_TraitTray",
						Animation = bulletPointPinIconAnimationName,
						Scale = Icons.Scale,
						Alpha = Icons.Alpha,
					})
					...
					Attach({
						Id = pinned.Id,
						DestinationId = screen.Components.RequirementsText.Id,
						OffsetX = listRequirementFormat.OffsetX - 10,
						OffsetY = startY + 1,
					})
				end
				...
				if config.IconInRequirements.Banned.Enable and reqBoonState == BoonState.Denied then
					local strikeThrough = CreateScreenComponent({
						Name = "BlankObstacle",
						Group = "Combat_Menu_TraitTray",
						Animation = "LockedKeepsakeIcon",
						Scale = Icons.Scale,
						Alpha = Icons.Alpha,
					})
					...
					Attach({
						Id = strikeThrough.Id,
						DestinationId = screen.Components.RequirementsText.Id,
						OffsetX = listRequirementFormat.OffsetX - 12,
						OffsetY = startY + 1,
					})
				end
				-- override END
		...
		local godPlate = CreateScreenComponent({
			Name = "BlankObstacle",
			Group = "Combat_Menu_TraitTray",
...
modutil.mod.Path.Override("ShowBoonInfoScreen", function(args)
	...
	ModifyTextBox({
		Id = components.TitleText.Id,
		Text = sourceData.BoonInfoTitleText or screen.CodexEntryData.BoonInfoTitle,
		LuaKey = "TempTextData",
		LuaValue = { BoonName = screen.LootName, WeaponName = screen.CodexEntryName },
	})
